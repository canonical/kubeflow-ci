...
from request_missing_tracks import get_charm_channel_map_for_applications, get_missing_tracks, Juju
...
def test_get_charm_channel_map_for_applications():
    ...
    # Arrange
    # Mock inputs
    applications = {f'app{i}': {'charm': f'someCharm{i}', 'channel': f'track{i}/channel{i}'} for i in range(3)}
...
def test_get_charm_channel_map_for_applications_for_non_existent_charm():
    ...
    # Arrange
    # Mock inputs
    applications = {f'app{i}': {'charm': f'someCharm{i}', 'channel': f'track{i}/channel{i}'} for i in range(1)}
...
def test_get_missing_tracks():
    ...
    charm_channel_map = {application['charm']: {f"{application['channel'].split('/')[0]}/someRandomRisk": None} for _, application in applications.items()}
